...
#ifndef __PF_STRINGSTREAM_HPP__
#define __PF_STRINGSTREAM_HPP__

#include "rdestl_common.hpp"
#include "rde_string.hpp"
#include "vector.hpp"
...
namespace pf
{
   ...
   *  - only supports signed int, signed long, float and pf::string
   *  - no support for unsigned word types
   *  - untested unicode support, no support for custom allocator
   */
  template< typename E, typename TAlloc = pf::allocator >
  struct basic_stringstream
  {
    typedef E                                   value_type;
    typedef pf::vector<value_type, TAlloc>     buffer_type;
    ...
    basic_stringstream& operator>>(int& x) {
      if (next()) x = atoi((const char*)current.c_str());
      return *this;
    }
    basic_stringstream& operator>>(long& x) {
      if (next()) x = atol((const char*)current.c_str());
      return *this;
    }
    basic_stringstream& operator>>(float& x) {
      if (next()) x = atof((const char*)current.c_str());
      return *this;
    }
    basic_stringstream& operator>>(pf::string& x) {
      if (next()) x = current;
      return *this;
    }
