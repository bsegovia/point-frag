...
#ifndef __PF_STL_FIXED_LIST_HPP__
#define __PF_STL_FIXED_LIST_HPP__
...
namespace pf
{
  namespace internal
  {
    ...
    template<int i>
    struct fixed_list_type_selector
    {
      typedef unsigned char index_type;
    };
    template<>
    struct fixed_list_type_selector<1>
    {
      typedef unsigned short index_type;
    };
    template<>
    struct fixed_list_type_selector<2>
    {
  ...
  class fixed_list
  {
    ...
    typedef internal::fixed_list_type_selector<select>  index_type_selector;
  ...
  public:
    ...
    const T& front() const  { PF_ASSERT(!empty()); return get_next(get_root())->value; }
    const T& back()  const  { PF_ASSERT(!empty()); return get_prev(get_root())->value; }
    T& front()              { PF_ASSERT(!empty()); return get_next(get_root())->value; }
    T& back()               { PF_ASSERT(!empty()); return get_prev(get_root())->value; }

    void push_front(const T& value)
    {
      PF_ASSERT(m_free_index_top > 0);  // 0 is root
      const int index = m_free_indices[m_free_index_top--];
      node* new_node = construct_node(value, index);
      ++m_num_nodes;
      PF_ASSERT(m_num_nodes <= TCapacity);
    ...
    void push_back(const T& value)
    {
      ...
      node* new_node = construct_node(value, index);
      ++m_num_nodes;
      PF_ASSERT(m_num_nodes <= TCapacity + 1);
    ...
    {
      PF_ASSERT(!empty());
      PF_ASSERT(it.node()->in_list());
      PF_ASSERT(it.list() == this);
...
#endif /* __PF_STL_FIXED_LIST_HPP__ */
