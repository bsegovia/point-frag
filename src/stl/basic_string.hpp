...
#ifndef __PF_STL_BASIC_STRING_HPP__
#define __PF_STL_BASIC_STRING_HPP__
...
namespace pf
{
  /*! @note: this one is totally _not_ std::string compatible for the time
   *  being! one way conversion should work, ie pf --> STL.
   */
  template<typename E,
           class TAllocator = pf::allocator,
           typename TStorage = pf::simple_string_storage<E, TAllocator> >
  ...
  public:
    ...
    {
      PF_ASSERT(end >= begin && end <= length() && begin >= 0);
      return basic_string(c_str() + begin, end - begin);
    }

    basic_string substr(size_type begin) const
    {
      PF_ASSERT(end >= begin && begin >= 0);
      return basic_string(c_str() + begin, 0);
    }
    ...
    const_iterator begin() const  { PF_ASSERT(invariant()); return c_str(); }
    const_iterator end() const    { PF_ASSERT(invariant()); return c_str() + length(); }
...
#endif // #ifndef __PF_STL_BASIC_STRING_HPP__
