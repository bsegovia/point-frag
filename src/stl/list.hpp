...
template<typename T, class TAllocator = pf::allocator>
class list
{
private:
...
public:
  ...
  const T& front() const  { PF_ASSERT(!empty()); return upcast(m_root.next)->value; }
  const T& back() const   { PF_ASSERT(!empty()); return upcast(m_root.prev)->value; }
  T& front()              { PF_ASSERT(!empty()); return upcast(m_root.next)->value; }
  T& back()               { PF_ASSERT(!empty()); return upcast(m_root.prev)->value; }
