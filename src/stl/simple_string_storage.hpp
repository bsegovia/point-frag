...
#ifndef __PF_SIMPLE_STRING_STORAGE_HPP__
#define __PF_SIMPLE_STRING_STORAGE_HPP__
...
namespace pf
{
  ...
  protected:
    ...
    {
      PF_ASSERT(m_data);
      PF_ASSERT(m_length <= m_capacity);
      if (length() != 0)
        PF_ASSERT(m_data[length()] == 0);
      return true;
    }
  private:
    value_type* construct_string(size_type capacity, size_type& out_capacity)
    {
      ...
      if (capacity != 0)
      {
        ...
        if ((unsigned long) capacity < kGranularity)
    ...
    void release_string()
    {
      if (m_capacity != 0)
      {
        PF_ASSERT(m_data != &m_end_of_data);
...
#endif /* __PF_SIMPLE_STRING_STORAGE_HPP__ */
