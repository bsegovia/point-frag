...
namespace pf
{
  ...
  template<typename TKey, typename TValue,
           class THashFunc = pf::hash<TKey>,
           int TLoadFactor4 = 6,
           class TKeyEqualFunc = pf::equal_to<TKey>,
           class TAllocator = pf::allocator>
  class hash_map
  {
  public:
    typedef pf::pair<TKey, TValue>         value_type;
  ...
  public:
    ...
    :  m_nodes(&ms_emptyNode),
      ...
      m_numUsed(0)
    {
      PF_ASSERT((kInitialCapacity & (kInitialCapacity - 1)) == 0);  // Must be power-of-two
    }
    ...
    void swap(hash_map& rhs)
    {
      ...
      {
        PF_ASSERT(invariant());
        PF_ASSERT(m_allocator == rhs.m_allocator);
        pf::swap(m_nodes, rhs.m_nodes);
        pf::swap(m_size, rhs.m_size);
        pf::swap(m_capacity, rhs.m_capacity);
        pf::swap(m_capacityMask, rhs.m_capacityMask);
        pf::swap(m_numUsed, rhs.m_numUsed);
        pf::swap(m_hashFunc, rhs.m_hashFunc);
        pf::swap(m_keyEqualFunc, rhs.m_keyEqualFunc);
        PF_ASSERT(invariant());
      }
    }

    pf::pair<iterator, bool> insert(const value_type& v)
    {
      typedef pf::pair<iterator, bool> ret_type_t;
      PF_ASSERT(invariant());
      ...
      if (n->is_occupied())
      {
        PF_ASSERT(hash == n->hash && m_keyEqualFunc(v.first, n->data.first));
        return ret_type_t(iterator(n, this), false);
      }
      if (n->is_unused())
        ++m_numUsed;
      pf::copy_construct(&n->data, v);
      n->hash = hash;
      ++m_size;
      PF_ASSERT(invariant());
  ...
  private:
    ...
    {
      PF_ASSERT((new_capacity & (new_capacity - 1)) == 0);  // Must be power-of-two
      ...
      m_nodes = newNodes;
      m_numUsed = m_size;
      PF_ASSERT(m_numUsed < m_capacity);
    }
    pf::pair<iterator, bool> insert_at(const value_type& v, node* n,
        hash_value_t hash)
    {
      PF_ASSERT(invariant());
      ...
      ++m_size;
      PF_ASSERT(invariant());
      return pf::pair<iterator, bool>(iterator(n, this), true);
    }
    ...
    node* lookup(const key_type& key) const
    {
      ...
      uint32 numProbes(0);
      // Guarantees loop termination.
      PF_ASSERT(m_capacity == 0 || m_numUsed < m_capacity);
      while (!n->is_unused())
      {
        ++numProbes;
        i = (i + numProbes) & m_capacityMask;
        ...
        int capacity, const node* nodes, bool destruct_original)
    {
      ...
      while (it != itEnd)
      {
        if (it->is_occupied())
        {
          ...
          pf::copy_construct(&n->data, it->data);
          n->hash = hash;
    ...
    INLINE hash_value_t hash_func(const key_type& key) const
    {
      ...
      //PF_ASSERT(h < node::kDeletedHash);
      return h;
    }
    bool invariant() const
    {
      PF_ASSERT((m_capacity & (m_capacity - 1)) == 0);
      PF_ASSERT(m_numUsed >= m_size);
      return true;
    }

    INLINE bool compare_key(const node* n, const key_type& key, hash_value_t hash,
        int_to_type<false>) const
    {
      return (n->hash == hash && m_keyEqualFunc(key, n->data.first));
    }
    INLINE bool compare_key(const node* n, const key_type& key, hash_value_t,
