...
#ifndef __PF_IMAGE_H__
#define __PF_IMAGE_H__
...
namespace pf
{
  ...
  INLINE Col3f toCol3f(const Col3f& c) { return c; }
  INLINE Col3f toCol3f(const Col3c& c) { return Col3f(c.r,c.g,c.b)*(1.0f/255.0f); }

  INLINE void setCol3f(Col3f& d, const Col3f& c) { d = c; }
  INLINE void setCol3f(Col3c& d, const Col3f& c) { d = Col3c(char(clamp(c.r)*255.0f),char(clamp(c.g)*255.0f),char(clamp(c.b)*255.0f)); }
  ...
  virtual ~ImageT() {
    ...
    INLINE const T  operator()(size_t x, size_t y) const { return data[y*width+x]; }
    INLINE       T& operator()(size_t x, size_t y)       { return data[y*width+x]; }
    INLINE Col3f get(size_t x, size_t y) const           { return toCol3f(data[y*width+x]); }
    INLINE void  set(size_t x, size_t y, const Col3f& c)        { setCol3f(data[y*width+x],c); }
    T* data;
  };
